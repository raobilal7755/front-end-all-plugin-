...
var pageLayout = {
	options: {
		fixedBlocks: ['header','footer'],
		scrollBlock: 'wrapper',
		mobileClass: 'mobile-scroll',
		desktopClass: 'desktop-scroll',
		forceMobile: true,
		hideAddressBar: true,
		fixFormsInScrollable: true,
		preventDocumentTouch: true,
		iScrollOptions: {onBeforeScrollStart: function(){}},
	...
	getElements: function() {
		this.scrollBlock = this.getObject(this.options.scrollBlock);
		this.scrollBarWidth = typeof scrollSize !== 'undefined' ? scrollSize.getWidth() : 0;
		this.fixedBlocks = [];
		if(typeof this.options.fixedBlocks === 'string') {
			this.fixedBlocks.push(this.getObject(this.options.fixedBlocks));
		} else {
			for(var i = 0; i < this.options.fixedBlocks.length; i++) {
				this.fixedBlocks.push(this.getObject(this.options.fixedBlocks[i]));
			}
		}
	},
	initScroll: function() {
		this.isMobile = this.options.mobileReg.test(navigator.userAgent) || this.options.forceMobile;
		this.fixLayout = this.isMobile ? this.fixMobileLayout : this.fixDesktopLayout;
		document.documentElement.className += ' ' + (this.isMobile ? this.options.mobileClass : this.options.desktopClass);
		this.fixLayout();
		if(this.isMobile) {
			this.iScrollLayout = new iScroll(this.scrollBlock, this.options.iScrollOptions);
		}
	},
	initAddressBar: function() {
		if(this.options.hideAddressBar && this.isMobile) {
			window.addEventListener('load', this.bind(this.hideAddressBar), false);
		}
	},
	hideAddressBar: function() {
		if(this.isMobile && !this.addressBarFlag) {
			this.addressBarFlag = true;
			var tmpHeight = this.scrollBlock.offsetHeight;
			this.scrollBlock.style.height = tmpHeight*3 + 'px';
			document.documentElement.style.overflow = document.body.style.overflow = 'auto';
			setTimeout(function(){window.scrollTo(0,1)},100);
			
			setTimeout(this.bind(function(){
				this.scrollBlock.style.height = tmpHeight + 'px'
				this.addressBarFlag = false;
				pageLayout.fixLayout();
			}),500);
		}
	},
	addEvents: function() {
		if (window.addEventListener) {
			if(this.options.fixFormsInScrollable) {
				var formElements = this.scrollBlock.querySelectorAll('input, select, textarea');
				for(var i = 0; i < formElements.length; i++) {
					formElements[i].addEventListener('touchstart',function(e){e.stopPropagation()},false);
					if(formElements[i].tagName === 'SELECT') {
						formElements[i].addEventListener('touchend',this.bind(function(e){
							var timer = setTimeout(this.bind(function(){
								this.scrollBlock.scrollTop = 0;
							}),1000);
						}),false);
					}
				}
			}
			if(this.options.preventDocumentTouch) {
				document.addEventListener('touchmove', function (e) { e.preventDefault(); }, false);
			}
			window.addEventListener('resize', this.bind(this.fixLayout), false);
			window.addEventListener('orientationchange', this.bind(this.fixLayout), false);
		} else {
			if (window.attachEvent) window.attachEvent('onresize', this.bind(this.fixLayout));
		}
	},
	fixMobileLayout: function() {
		if(!this.addressBarFlag) {
			if(this.scrollBlock) {
				this.fixedSumHeight = 0;
				for(var i = 0 ; i < this.fixedBlocks.length; i++) {
					if(this.fixedBlocks[i]) {
						this.fixedSumHeight += this.fixedBlocks[i].offsetHeight;
					}
				}
				this.scrollBlock.style.height = (window.innerHeight - this.fixedSumHeight) + 'px';
			}
			if(this.iScrollLayout) {
				this.iScrollLayout.refresh();
			}
		}
	},
	fixDesktopLayout: function() {
		if(this.scrollBlock) {
			this.scrollBlock.style.overflowX = 'hidden';
			for(var i = 0 ; i < this.fixedBlocks.length; i++) {
				if(this.fixedBlocks[i]) {
					this.fixedBlocks[i].style.width = (this.scrollBlock.offsetWidth - this.scrollBarWidth) + 'px';
				}
			}
		}
	},
...
// default scrollbar dimensions detector
